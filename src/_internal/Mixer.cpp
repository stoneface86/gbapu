...
void Mixer::mixfast(int8_t delta, uint32_t cycletime) {
    ...
    if constexpr (modePansLeft(mode)) {
        blip_add_delta(mBlip.bbuf[0], cycletime, (int16_t)((delta * mVolumeStepLeft + 0x8000) >> 16));
    }
    if constexpr (modePansRight(mode)) {
        blip_add_delta(mBlip.bbuf[1], cycletime, (int16_t)((delta * mVolumeStepRight + 0x8000) >> 16));
    }
}
